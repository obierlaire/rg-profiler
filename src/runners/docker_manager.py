...
def build_framework_image(framework_dir, image_name, db_type, mode=MODE_PROFILE):
    ...
    try:
        ...
        scalene_cmd = f"scalene --json --profile-all --outfile=/output/scalene/scalene.json --reduced-profile app.py"
        ...
        if mode == MODE_ENERGY:
            ...
            final_content = template_content.replace("{{RUN_COMMAND}}", run_cmd)
        elif mode == MODE_STANDARD:
            # For standard mode, use scalene
            final_content = template_content.replace("{{RUN_COMMAND}}", scalene_cmd)
        else:
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.dockerfile') as tmp:
        ...
        try:
            ...
            dockerfile_content = template_content.replace("{{PYTHON_VERSION}}", python_version)
            dockerfile_content = dockerfile_content.replace("{{DB_TYPE}}", db_type)
            dockerfile_content = dockerfile_content.replace("{{DB_PORT}}", str(db_port))
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "build", "-t", image_name, "-f", dockerfile_path, temp_context_dir],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        if framework_entrypoint.exists():
            try:
                ...
                print(f"Warning: Could not remove framework entrypoint.sh: {e}")
...
def run_framework_container(image_name, output_dir, framework_config, mode=MODE_PROFILE):
    ...
    try:
        ...
        print(f"✅ Started container {container_name} with ID: {container_id[:12]}")
        ...
        try:
            ...
            if not result.stdout.strip():
                ...
                if exists_result.stdout.strip():
                    ...
                    print(f"⚠️ Container exited with code {exit_code}, getting logs...")
            ...
            if has_curl:
                ...
                for endpoint in ["/json", "/plaintext", "/", "/health"]:
                    try:
                        print(f"🔍 Checking internal endpoint: http://localhost:{server_port}{endpoint}")
            ...
            print(f"⏳ Waiting for server to be ready... ({int(time.time() - start_time)}/{timeout}s)")
            time.sleep(2)
